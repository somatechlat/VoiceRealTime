...
class OnboardingStatus(BaseModel):
    ...
    tenant_id: str
    milestones: dict[str, Optional[datetime]] = Field(
        description="Milestone completion timestamps"
    )
...
async def signup(
    ...
    try:
        ...
        await lago.create_customer(
            ...
            timezone="UTC",
            metadata=[
                {"key": "use_case", "value": request.use_case.value if request.use_case else "other"},
    ...
    try:
        # 4. Generate first API key
        from ....app.services.api_key_service import generate_api_key, APIKeyHasher
