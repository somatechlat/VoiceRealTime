...
class StripeProvider(PaymentProviderBase):
    ...
    async def confirm_payment(self, payment_id: str) -> Payment:
        ...
        response = await self._request(
            "POST", f"/payment_intents/{payment_id}/confirm"
        )
        response.raise_for_status()
    ...
    async def cancel_payment(self, payment_id: str) -> Payment:
        ...
        response = await self._request(
            "POST", f"/payment_intents/{payment_id}/cancel"
        )
        response.raise_for_status()
    ...
    async def detach_payment_method(self, payment_method_id: str) -> None:
        ...
        response = await self._request(
            "POST", f"/payment_methods/{payment_method_id}/detach"
        )
        response.raise_for_status()
...
class PayPalProvider(PaymentProviderBase):
    ...
    async def confirm_payment(self, payment_id: str) -> Payment:
        ...
        response = await self._request(
            "POST", f"/v2/checkout/orders/{payment_id}/capture"
        )
        response.raise_for_status()
    ...
    async def list_payment_methods(
        ...
        for token in response.json().get("payment_tokens", []):
            ...
            methods.append(PaymentMethod(
                ...
                brand=card.get("brand"),
                exp_month=int(card.get("expiry", "01/2000").split("/")[0]) if card.get("expiry") else None,
                exp_year=int(card.get("expiry", "01/2000").split("/")[1]) if card.get("expiry") else None,
            ))
    ...
    async def detach_payment_method(self, payment_method_id: str) -> None:
        ...
        response = await self._request(
            "DELETE", f"/v3/vault/payment-tokens/{payment_method_id}"
        )
