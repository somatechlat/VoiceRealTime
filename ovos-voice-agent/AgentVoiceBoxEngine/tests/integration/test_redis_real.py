...
class TestDistributedSessionAcrossGateways:
    ...
    @pytest.mark.asyncio
    async def test_conversation_items_shared_across_gateways(
        self, redis_client, redis_client_2
    ):
...
class TestRateLimiterConcurrentLoad:
    ...
    async def test_rate_limit_accuracy_100_concurrent_requests(self, redis_client):
        ...
        assert allowed_count >= 45, (
            f"Allowed only {allowed_count} requests, expected at least 45"
        )
        assert denied_count >= 48, (
            f"Only denied {denied_count} requests, expected at least 48"
        )

    @pytest.mark.asyncio
    async def test_rate_limit_across_multiple_gateways(
        self, redis_client, redis_client_2
    ):
...
class TestRedisStreamsConsumerGroups:
    ...
    async def test_multiple_consumers_in_group(self, redis_client, redis_client_2):
        ...
        streams_client2 = RedisStreamsClient(redis_client_2)
